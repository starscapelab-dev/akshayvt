  ...
  return (
    ...
    >
      <Glass
        hover={false}
        className={cn(
          "px-[22px] pt-6 pb-[22px] h-full flex flex-col",
      ...
      >
        ...
        <div className="flex items-start justify-between mb-5">
          <div
            className={cn(
              "w-[38px] h-[38px] rounded-xl grid place-items-center text-ink flex-none",
        ...
        {/* Title */}
        <h3 className="text-[17.5px] font-medium tracking-tight m-0 mb-2">{title}</h3>

        {/* Description - flex grow to push footer down */}
        <p className="text-[13px] text-ink-2 m-0 leading-[1.55] flex-1">{description}</p>

        {/* Footer: Tags + Link */}
        <div className="flex items-center justify-between mt-5 pt-4 border-t border-white/[.06]">
          {tags && (
            <span className="mono text-[10px] text-muted tracking-[.08em] uppercase">{tags}</span>
          )}
          <span className="mono inline-flex items-center gap-1.5 text-[11px] tracking-wide text-muted uppercase transition-all duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] group-hover:text-accent group-hover:gap-2 ml-auto">
            View
